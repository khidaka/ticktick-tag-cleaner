...
| 処理 | 対象 | 動作 |
|---|---|---|
| **ピン解除** | ピン留めされた全タスク | `sortOrder` を 0 にリセット（TickTick はピンを極端な負の sortOrder で表現） |
...
├─ 2. ピン解除（全タスク対象）
│     └─ sortOrder < -1兆 のタスク全件
│           → sortOrder を 0 にリセット（アンピン）
│
├─ 3. 繰り返しタスク（repeatFlag === true）を走査
...
│
└─ 4. 処理結果を表示
      （ピン解除件数、タグ削除件数、新規遅延件数、遅延インクリメント件数）
```
...
const PIN_SORT_ORDER_THRESHOLD = -1_000_000_000_000;  // ピン判定の閾値（-1兆以下）
```

### ピン解除のしくみ

TickTick は boolean フィールド（`isPin`など）ではなく、`sortOrder` **の極端な負の値**（例：`-50577535926272`）でピン状態を表現しています。

- **ピン済み**: `sortOrder < -1,000,000,000,000`（-1兆未満）
- **アンピン処理**: `sortOrder` を `0` にリセット

診断スクリプト（`TickTickDiagnostic.js`）で、ピン済みタスクの実際の `sortOrder` 値を確認できます。

### sortOrder の閾値を調整する場合

デフォルト（-1兆）でほぼすべてのピン済みタスクを検出できます。必要に応じて `PIN_SORT_ORDER_THRESHOLD` の値を変更してください。
...
| ピン解除が動かない | `PIN_SORT_ORDER_THRESHOLD` の値を調整するか、`TickTickDiagnostic.js` で実際の sortOrder 値を確認 |
...
| バージョン | 日付 | 変更内容 |
|---|---|---|
| v1.9.0 | 2026-04-17 | ピン機構を修正：sortOrder < -1兆 でピンを検出、sortOrder: 0 でアンピン |
